...
public interface ExpenseDao {
    ...
    @Query("SELECT * FROM expenses WHERE user_id = :userId AND " +
            "CAST(strftime('%m', datetime(date/1000, 'unixepoch')) AS INTEGER) = :month AND " +
            "CAST(strftime('%Y', datetime(date/1000, 'unixepoch')) AS INTEGER) = :year " +
            "ORDER BY date DESC")
